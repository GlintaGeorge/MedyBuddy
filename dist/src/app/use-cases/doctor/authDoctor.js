...
const addNewDoctor = (doctorData, doctorRepository, authService) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const doctor = (0, doctorEntity_1.default)(doctorName, email, hashedPassword, verificationToken, phoneNumber, department, education, description, experience, rejectedReason, lisenceCertificate);
    const createdDoctor = yield doctorRepository.addDoctor(doctor);
    console.log(verificationToken, "token");
    //   sent verification mail to Doctor email address
    if (createdDoctor) {
...
const authenticateGoogleSignInUser = (doctorData, doctorDbRepository, authService) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (isEmailExist) {
        const { accessToken, refreshToken } = authService.createTokens(isEmailExist.id, isEmailExist.doctorName, isEmailExist.role);
        return { accessToken, isEmailExist, refreshToken };
    }
    else {
        ...
        const { accessToken, refreshToken } = authService.createTokens(userId, createdUser.doctorName, createdUser.role);
